		...
		if ($this->settings[$model->alias]['auto_bind']) {
		    if ($hasFieldCreatedBy) {
				$commonBelongsTo = array(
					Inflector::classify($this->settings[$model->alias]['created_by_field']) => array(
						'counterCache' => strtolower($model->alias) . "_count",
			...
			if ($hasFieldModifiedBy) {
				$commonBelongsTo = array(
					Inflector::classify($this->settings[$model->alias]['modified_by_field']) => array(
						'className' => $this->settings[$model->alias]['user_model'],
 ...
 **/
	function beforeValidate(&$model) {
		...
		if (empty($model->data[$model->alias][$model->primaryKey])) {
			$model->data[$model->alias][$settings['created_by_field']] = Authsome::get('id');
		}
		$model->data[$model->alias][$settings['modified_by_field']] = Authsome::get('id');
		return true;
	}
}
?>